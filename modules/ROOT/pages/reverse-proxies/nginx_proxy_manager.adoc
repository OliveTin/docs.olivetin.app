...
[source,yaml]
----
include::etc/npm-docker-compose.yml[]
----

Note that OliveTin needs a configuration file to run, see the <<install-compose,docker compose install instructions>> for a bit more detail.
...
That really should be all that you need to get OliveTin working with Nginx Proxy Manager. If you have any issues, please check the logs of both OliveTin and Nginx Proxy Manager for any errors, and look for ways of getting <<support,support>>.
