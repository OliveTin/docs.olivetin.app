...
* `ot_username` — The username of the user who started the execution. In templates use `{{ ot_username }}`; in the process environment it is `OT_USERNAME`. For unauthenticated or automated runs this may be `guest`, `cron`, or similar, depending on how the action was triggered.
* `ot_executionTrackingId` — A unique identifier for this execution. In templates use `{{ ot_executionTrackingId }}`; in the process environment it is `OT_EXECUTIONTRACKINGID`. Useful for logging, correlating with the API or execution log, or idempotency in scripts.

Example in a shell command:

[source,yaml]
----
shell: echo "Run by {{ ot_username }} (execution {{ ot_executionTrackingId }})"
----
...
In xref:action_execution/aftercompletion.adoc[Execute after completion] (`shellAfterCompleted`), the same names are available as template variables (e.g. `{{ ot_username }}`, `{{ ot_executionTrackingId }}`); user-defined argument values are not passed there.
