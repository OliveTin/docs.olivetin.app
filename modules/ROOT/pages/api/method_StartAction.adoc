...
.curl
----
user@host: curl "http://olivetin.webapps.teratan.lan/api/StartAction" --json '{"actionId": "nuclear_reactor_shutdown"}'
----
...
.curl
----
user:host: curl 'http://olivetin.example.com/api/StartAction' --json '{"actionId": "Ping_host", "arguments": [{"name": "host", "value": "example.com"},{"name": "count", "value": "1"}]}'
----
