...
If you are trying to integrate OliveTin with your own scripts or processes, it's probably easiest to start actions by using an alias directly in the URL, <<api-eg-startAlias,see the example>>.
...
[#api-eg-startAlias]
==== Example API call; Start an action by Alias

.curl
----
user@host: curl http://olivetin.example.com/api/StartActionByAlias/pingGithub
----
