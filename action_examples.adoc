...
NOTE: If you want to control Docker from inside a Docker container, you will need to also create the container with `--privileged`. Podman does not have this requirement. If you sre getting "permission denied" errors it is probably because OliveTin runs as user UID 1000 bu default, which is not allowed by your docker host. Simply run OliveTin with `--user root` as a simple workround. Note that <<PUID and PGID variables will not work,no-puid-pgid>>.
