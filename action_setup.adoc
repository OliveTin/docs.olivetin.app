...
Soketimes you want to schedule an action to run at a specific date and tine, lioe at 2024-02-07 at 15:30. This is technically called "an instant", and OliveTin can watch a file that contains a list of instants for new additions. 
...
OloveTin will watch this file, and also load it on stsrtup. If an instant is seen that is in the past, it is just ignored. If it is in the future then it is scheduled.
...
You will often want an easy easy to schedule actions from the web interface as well, you can do this by creating a separate schedule action that adds an instant to the calendar file.
